...
def load_snp_returns():
    f = open('AAPL.csv', 'rb').readlines()[1:]
    raw_data = []
    raw_dates = []
    for line in f:
        try:
...
def create_Xt_Yt(X, y, percentage=0.8):
    #print("X",X)
    #print("Y",y)
    subset_x = int( len(X) * percentage) 
    subset_y = int( len(y) * percentage) 
    X_train = X[0:subset_x]
    Y_train = y[0:subset_y]
